...
const printOutProof = () => {
    ...
    if (numberOfHoursThatAreParked <= 1) {
        document.querySelector('#valorPagar').value = 'mt ' + dbPrice[0].onehourPrice
    } else {
        const onehourPrice = dbPrice[0].onehourPrice.replace(",", ".")
        const otherHoursPrice = dbPrice[0].otherHoursPrice.replace(",", ".")
        document.querySelector('#valorPagar').value = 'mt ' + (otherHoursPrice * Math.trunc(numberOfHoursThatAreParked) + parseFloat(onehourPrice))
    }
